...
impl Particle {
    pub fn new(cam_pos: Point3<f32>) -> Particle {
        ...
        Particle {
            position: Point3::new(
                cam_pos.x + radius * Rad::sin(angle),
...
impl Weather {
    ...
    pub fn draw<S: glium::Surface>(&mut self, surface: &mut S, camera: &Camera) {
        ...
        match self.form {
            Form::Snow => {
                ...
                scaling = Matrix4::from_cols(
                    Vector4f::new(0.05, 0.0, 0.0, 0.0),
        ...
        surface
            .draw(
                (&self.vertex_buffer, self.actual_buf.per_instance().unwrap()),
                &self.indices,
                &self.program,
                &uniforms,
                &params,
    ...
    pub fn update(
        ...
        for particle in &mut self.particles.iter_mut() {
            ...
            match self.form {
                ...
                Form::Rain => {
                    ...
                    particle.position.z =
                        particle.position.z - ((particle.velocity + 0.5) * 50.0 * delta)
                }
        ...
        let vertex_buffer =
            glium::VertexBuffer::new(self.context.get_facade(), &tmp_instances).unwrap();
